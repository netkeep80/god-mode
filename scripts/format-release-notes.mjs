...
try {
  ...
  if (commitShaToLookup) {
    ...
    console.log(`ℹ️ Looking up PR for commit ${commitShaToLookup} (from ${source})`)

    try {
      const prResult = await $`gh api "repos/${repository}/commits/${commitShaToLookup}/pulls"`.run(
        { capture: true }
      )
      ...
      if (relevantPr) {
        ...
        console.log('⚠️ Found PRs but all are version bump PRs, not linking any')
      } else {
        console.log('ℹ️ No PR found containing this commit - not adding PR link')
      }
    } catch (error) {
