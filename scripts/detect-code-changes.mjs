...
function detectChanges() {
  ...
  // Detect .ts/.tsx file changes
  const tsChanged = changedFiles.some((file) => file.endsWith('.ts') || file.endsWith('.tsx'))
  setOutput('ts-changed', tsChanged ? 'true' : 'false')
  ...
  // Check if any code files changed (.ts, .tsx, .mjs, .js, .json, .yml, .yaml, or workflow files)
  const codePattern = /\.(ts|tsx|mjs|js|json|yml|yaml)$|\.github\/workflows\//
